...
def get_dsl(prompter, variables, seed=42):
    ...
    def __obtain_objects_from_img(img, objects, properties):
        ...
        prompt = f"""
        ## Task
        Identify objects and their properties from the scene description using only the provided lists.

        **Objects:** {objects}
        **Properties:** {properties}

        ## Scene Description Format
        Each item has the format: item(ID, color, shape, orientation, interaction)
        - ID: integer identifier
        - color, shape, orientation: attributes of the object
        - interaction: how the object relates to others (e.g. grounded, touching(ID), on_top_of(ID))
        One entry per item in the scene.

        ## Rules
        1. Only use objects/properties from the provided lists
        2. Return one inner list per item in the scene
        ...
        **Example 1**
        - Objects: ["block", "wedge", "pyramid"]
        - Properties: ["blue", "red", "upright", "flat", "grounded"]
        - Scene Description: ['item(0, red, block, upright, grounded)', 'item(1, blue, wedge, flat, touching(0))', 'item(2, red, pyramid, upright, grounded)']

        ```python
        objects = [
            ['block', 'red', 'upright', 'grounded'],
            ['wedge', 'blue', 'flat'],
            ['pyramid', 'red', 'upright', 'grounded']
        ]
        ```

        **Example 2**
        - Objects: ["block", "pyramid"]
        - Properties: ["blue", "upright", "upside_down"]
        - Scene Description: ['item(0, blue, block, upright, grounded)', 'item(1, blue, block, upside_down, on_top_of(0))']

        ```python
        objects = [
            ['block', 'blue', 'upright'],
            ['block', 'blue', 'upside_down']
        ]
        ```

        **Analyze the scene description now:**
        {img}
        """

        response = prompter.prompt_with_text(
            prompt_text=prompt, max_new_tokens=1000, seed=seed
        )
    ...
    def __obtain_actions_from_img(img, objects, properties, actions):
        ...
        prompt = f"""
        ## Task
        Identify interaction triples from the scene description using only the provided lists.
        Each triple has the format [subject, action, object] where subject and object are the
        shape or color of the items involved (from Objects or Properties), and action is from Actions.
        ...
        **Actions:** {actions}
        ...
        ## Scene Description Format
        Each item has the format: item(ID, color, shape, orientation, interaction)
        - The interaction field encodes what this item is doing, e.g.:
          - grounded: item rests on the ground (not a pairwise interaction)
          - touching(ID): this item is touching the item with the given ID
          - on_top_of(ID): this item is on top of the item with the given ID
          - pointing(ID): this item is pointing at the item with the given ID
        For pairwise interactions (touching, on_top_of, pointing), look up the other item by ID
        to determine its shape/color, then form the triple [subject_shape_or_color, action, object_shape_or_color].

        ## Rules
        1. Only use values from the provided lists for each position
        2. Each triple must have exactly 3 elements
        3. One triple per pairwise interaction found in the scene
        4. If no action from the Actions list appears, return `actions = [[]]`
        ...
        **Example 1**
        ...
        - Properties: ["blue", "red"]
        - Actions: ["touching", "on_top_of"]
        - Scene Description: ['item(0, blue, block, upright, grounded)', 'item(1, red, pyramid, upright, touching(0))', 'item(2, blue, block, upright, touching(0))']

        item 1 (red pyramid) is touching item 0 (blue block) → ['pyramid', 'touching', 'block'], ['red', 'touching', 'block'], ['pyramid', 'touching', 'blue'], ['red', 'touching', 'blue']
        item 2 (blue block) is touching item 0 (blue block) → ['block', 'touching', 'block'], ['blue', 'touching', 'block'], ['block', 'touching', 'blue'], ['blue', 'touching', 'blue']

        ```python
        actions = [
            ['pyramid', 'touching', 'block'],
            ['red', 'touching', 'block'],
            ['pyramid', 'touching', 'blue'],
            ['red', 'touching', 'blue'],
            ['block', 'touching', 'block'],
            ['blue', 'touching', 'block'],
            ['block', 'touching', 'blue'],
            ['blue', 'touching', 'blue']
        ]
        ...
        **Example 2**
        ...
        - Properties: ["blue", "red"]
        - Actions: ["touching", "on_top_of"]
        - Scene Description: ['item(0, red, block, upright, grounded)', 'item(1, blue, wedge, doorstop, on_top_of(0))']

        item 1 (blue wedge) is on_top_of item 0 (red block) → ['wedge', 'on_top_of', 'block'], ['blue', 'on_top_of', 'block'], ['wedge', 'on_top_of', 'red'], ['blue', 'on_top_of', 'red']

        ```python
        actions = [
            ['wedge', 'on_top_of', 'block'],
            ['blue', 'on_top_of', 'block'],
            ['wedge', 'on_top_of', 'red'],
            ['blue', 'on_top_of', 'red']
        ]
        ```
        ...
        **Example 3**
        - Objects: ["block"]
        - Properties: ["blue", "upright"]
        - Actions: ["touching"]
        - Scene Description: ['item(0, blue, block, upright, grounded)', 'item(1, blue, block, upright, grounded)']

        No item has a touching/on_top_of/pointing interaction — both are grounded (not pairwise).
