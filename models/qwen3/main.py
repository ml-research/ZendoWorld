...
# Per-model sampling kwargs passed to model.generate(). For Qwen3.5 we
# follow the user's specified config (temperature=1.0, top_p=0.95). For
# the older Qwen3-VL entries we keep the original behaviour where the
# ``sampling`` constructor arg controls do_sample (greedy by default).
_SAMPLING_KWARGS_QWEN35 = {"do_sample": True, "temperature": 1.0, "top_p": 0.95}
...
class Qwen3Prompter:
    def __init__(
        ...
        # Seed once at construction; do NOT re-seed per call. Per-call seed
        # resetting collapsed sampling diversity across turns even when the
        # input set of example images changed (same issue as documented in
        # models/gemma3/main.py). Run-level reproducibility is preserved by
        # the global seed set here.
        torch.manual_seed(seed)
        torch.cuda.manual_seed_all(seed)
        self.seed = seed

        # Per-model kwargs passed to apply_chat_template. For the Qwen3 line,
        # `enable_thinking=False` skips the <think> preamble so the model goes
        # straight to the answer; on the Instruct variants it's a no-op since
        # they don't have a thinking template anyway. The Thinking variant is
        # left alone because the whole point of that model is to think.
        if model == "Qwen3-VL-30B-A3B-Instruct":
            self.model_name = "Qwen/Qwen3-VL-30B-A3B-Instruct"
            self._sample_kwargs = {"do_sample": sampling}
            self._chat_kwargs = {}
            ...
            self._sample_kwargs = dict(_SAMPLING_KWARGS_QWEN35)
            self._chat_kwargs = {"enable_thinking": False}
            # Qwen3.5 always samples regardless of the constructor arg; keep
            # self.sampling consistent so the memory_file suffix and print
            # below reflect actual behaviour.
            sampling = True
        else:
            raise ValueError(
                f"Model {model} not supported. Use 'Qwen3-VL-30B-A3B-Instruct', "
                "'Qwen3-VL-30B-A3B-Thinking', or 'Qwen3.5-27B'."
            )
    ...
    def _load_model(self):
        # AutoModelForImageTextToText auto-detects the underlying class from
        # config.json, so the same loader path covers Qwen3-VL MoE and the
        # dense Qwen3.5 line without re-importing per architecture.
    ...
    def prompt_with_text(
        self, prompt_text, use_memory=True, max_new_tokens=1500, overwrite_memory=False, seed=None
    ):

        # Intentionally NO per-call seed reset — see note in __init__.
        ...
        generated_ids = self.model.generate(
            **inputs, max_new_tokens=max_new_tokens, **self._sample_kwargs
        )
        generated_ids_trimmed = [
            out_ids[len(in_ids) :]
            for in_ids, out_ids in zip(inputs.input_ids, generated_ids)
        ]
        n_new = sum(int(t.shape[0]) for t in generated_ids_trimmed)
    ...
    def prompt_with_images(
        ...
        with torch.inference_mode():
            ...
            #     for in_ids, out_ids in zip(inputs.input_ids, generated_ids)
            # ]
            # 3. Trim input tokens from outputs
            generated_ids_trimmed = [
                out_ids[len(in_ids) :]
                for in_ids, out_ids in zip(inputs.input_ids, generated.sequences)
            ]

            # 4. Decode — keep special tokens so <think> shows up
        ...
        # 5. Extract reasoning and final output
        output_tokens = 0
